...
(section xref:flow-level-transitions[8.9.5]).
...
xref:supporting-classes[10.8] for more on restart processing.
...
See the example at the end of section xref:flow-level-transitions[8.9.5] for further clarification.
...
section xref:partitionplan[10.8.4] has an assertion in rule 3.e. that suggests using the
...
itself is currently UNDEFINED (section xref:flow-level-transitions[8.9.5])
